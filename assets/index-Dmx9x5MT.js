        ...
        <tbody>
          ${e.map((e,t)=>`
            <tr class="note-row" data-index="${t}">
              <td>${K(e.owner)}/${K(e.repo)}</td>
              <td><a href="${$(Z(E.host,e.owner,e.repo,e.number))}" target="${Q(Z(E.host,e.owner,e.repo,e.number))}" class="issue-link" onclick="event.stopPropagation()">${e.number}</a></td>
              <td>${K(e.title)}</td>
              <td>${new Date(e.updated_at).toLocaleDateString(`en-US`,{month:`numeric`,day:`numeric`,year:`2-digit`})}</td>
              <td><button class="copy-url-btn" data-url="${$(Z(E.host,e.owner,e.repo,e.number))}" title="Copy issue URL">${q}</button></td>
              <td><button class="context-menu-btn" data-index="${t}" title="More actions">&#x2026;</button></td>
            </tr>
          `).join(``)}
        </tbody>
      </table>
    `,t.querySelectorAll(`.copy-url-btn`).forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=e.dataset.url;navigator.clipboard.writeText(n).then(()=>{e.innerHTML=J,setTimeout(()=>{e.innerHTML=q},1500)})})}),t.querySelectorAll(`.context-menu-btn`).forEach(n=>{n.addEventListener(`click`,r=>{r.stopPropagation(),document.querySelector(`.note-context-menu`)?.remove();let i=parseInt(n.dataset.index,10),a=e[i],o=n.getBoundingClientRect(),s=document.createElement(`div`);s.className=`note-context-menu`,s.innerHTML=`
          <button class="context-github-btn">${X} Edit on GitHub</button>
          <button class="context-delete-btn">${Y} Delete</button>
        `,s.style.top=`${o.bottom+4}px`,s.style.left=`${o.right}px`,document.body.appendChild(s);let l=()=>{s.remove(),document.removeEventListener(`click`,l)};setTimeout(()=>document.addEventListener(`click`,l),0),s.querySelector(`.context-github-btn`).addEventListener(`click`,e=>{e.stopPropagation(),l(),window.open(Z(E.host,a.owner,a.repo,a.number)+`#new_comment_field`,`_blank`)}),s.querySelector(`.context-delete-btn`).addEventListener(`click`,async e=>{e.stopPropagation(),l();try{await c(E.host,E.token,a.owner,a.repo,a.number),t.querySelector(`.note-row[data-index="${i}"]`)?.remove()}catch(e){alert(`Failed to delete note: ${e instanceof Error?e.message:e}`)}})})}),t.querySelectorAll(`.note-row`).forEach(t=>{t.addEventListener(`click`,()=>{let n=parseInt(t.getAttribute(`data-index`),10),r=e[n];h?window.open(Z(E.host,r.owner,r.repo,r.number)+`#new_comment_field`,`_blank`):B(r.owner,r.repo,r.number)})}),r()}catch(e){document.getElementById(`notes-container`).innerHTML=`<p class="error">Failed to load notes: ${e instanceof Error?e.message:e}</p>`}}function R(e){document.getElementById(`repo-picker-overlay`)?.remove();let t=new Map,n=C();if(n){let[e,r]=n.split(`/`);t.set(n,{owner:e,repo:r})}for(let n of e){let e=`${n.owner}/${n.repo}`;t.has(e)||t.set(e,{owner:n.owner,repo:n.repo})}let r=Array.from(t.entries()).sort((e,t)=>{if(n){if(e[0]===n)return-1;if(t[0]===n)return 1}return e[0].localeCompare(t[0])}),i=document.createElement(`div`);i.id=`repo-picker-overlay`,i.innerHTML=`
    <div class="repo-picker">
      <h2>Select repository</h2>
      <div class="repo-list">
        ${r.map(([e,t])=>`
          <button class="repo-option" data-owner="${$(t.owner)}" data-repo="${$(t.repo)}">${K(e)}</button>
        `).join(``)}
      </div>
  ...
  `,M.appendChild(i),i.addEventListener(`click`,e=>{e.target===i&&i.remove()});let a=e=>{e.key===`Escape`&&(i.remove(),document.removeEventListener(`keydown`,a))};document.addEventListener(`keydown`,a),i.querySelectorAll(`.repo-option`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.owner,n=e.dataset.repo;i.remove(),document.removeEventListener(`keydown`,a),z(t,n)})});let o=document.getElementById(`repo-other-go`),s=document.getElementById(`repo-other-input`),c=()=>{let e=s.value.trim().split(`/`);if(e.length!==2||!e[0]||!e[1]){s.classList.add(`error`);return}i.remove(),document.removeEventListener(`keydown`,a),z(e[0],e[1])};o.addEventListener(`click`,c),s.addEventListener(`keydown`,e=>{e.key===`Enter`&&c()})}function z(e,t){if(h){window.open(`https://${E.host}/${e}/${t}/issues/new`,`_blank`);return}O={owner:e,repo:t},D=null,k=S,A=x,V(x,S)}async function B(e,t,n){if(E){M.innerHTML=`<div class="editor-screen"><p>Loading note #${n}...</p></div>`;try{let r=await a(E.host,E.token,e,t,n);D={...r,owner:e,repo:t},k=r.body??``,A=r.title,V(r.title,k)}catch(e){M.innerHTML=`<div class="editor-screen"><p class="error">Failed to load note: ${e instanceof Error?e.message:e}</p></div>`}}}function V(e,t){I?.(),I=null,M.innerHTML=`
    <div class="editor-screen">
      <header>
        ...
        <input type="text" id="note-title" value="${$(e)}" />
        <span id="note-number">${D?`<a href="${$(Z(E.host,D.owner,D.repo,D.number))}" target="${Q(Z(E.host,D.owner,D.repo,D.number))}" class="issue-link">#${D.number}</a>`:`Title`}</span>
        ${D?`<button id="copy-note-url" class="copy-url-btn" title="Copy issue URL">${q}</button>`:``}
        <span id="status-msg"></span>
      </header>
      <div id="editor-container"></div>
    </div>
  `,document.getElementById(`back-to-list`).addEventListener(`click`,()=>{U(!1)});let n=document.getElementById(`copy-note-url`);if(n&&D){let e=Z(E.host,D.owner,D.repo,D.number);n.addEventListener(`click`,()=>{navigator.clipboard.writeText(e).then(()=>{n.innerHTML=J,setTimeout(()=>{n.innerHTML=q},1500)})})}let r=document.getElementById(`note-title`);r.addEventListener(`keydown`,e=>{(e.key===`Escape`||e.key===`Enter`)&&(e.preventDefault(),_.focusEditor())}),_.createEditor(document.getElementById(`editor-container`),t,{onSave:H,onQuit:U},{storagePrefix:`notehub`,normalMappings:{gt:()=>r.focus()}})}async function H(){if(!E)return;let e=_.getEditorContent(),t=document.getElementById(`note-title`).value.trim();if(!D&&O){if(!t){G(`Title required`,!0);return}try{G(`Creating...`);let n=await s(E.host,E.token,O.owner,O.repo,t,e);D={...n,owner:O.owner,repo:O.repo},j=D,O=null,k=n.body??``,A=n.title;let r=document.getElementById(`note-number`);r&&(r.innerHTML=`<a href="${$(Z(E.host,D.owner,D.repo,D.number))}" target="${Q(Z(E.host,D.owner,D.repo,D.number))}" class="issue-link">#${D.number}</a>`),G(`Created`)}catch(e){G(`Create failed: ${e instanceof Error?e.message:e}`,!0)}return}if(!D)return;let n={};if(e!==k&&(n.body=e),t!==A&&(n.title=t),Object.keys(n).length===0){G(`No changes`);return}try{G(`Saving...`);let e=await o(E.host,E.token,D.owner,D.repo,D.number,n);k=e.body??``,A=e.title,D={...e,owner:D.owner,repo:D.repo},G(`Saved`)}catch(e){G(`Save failed: ${e instanceof Error?e.message:e}`,!0)}}function U(e){let t=document.getElementById(`note-title`),n=t?t.value.trim()!==A:!1;if(!e&&(_.isEditorDirty(k)||n)){W(`Unsaved changes. Discard?`,()=>L());return}L()}function W(e,t){document.getElementById(`confirm-bar`)?.remove();let n=document.createElement(`div`);n.id=`confirm-bar`,n.innerHTML=`
    <span>${e}</span>
    <span class="confirm-hint">[y]es / [n]o</span>
    <button class="confirm-btn confirm-yes">Yes</button>
    <button class="confirm-btn confirm-no">No</button>
  `;let r=document.querySelector(`.editor-screen header`);if(!r)return;r.after(n);let i=()=>{n.remove(),document.removeEventListener(`keydown`,a,!0)},a=e=>{e.key===`y`||e.key===`Enter`?(e.stopPropagation(),e.preventDefault(),i(),t()):(e.key===`n`||e.key===`Escape`)&&(e.stopPropagation(),e.preventDefault(),i())};document.addEventListener(`keydown`,a,!0),n.querySelector(`.confirm-yes`).addEventListener(`click`,()=>{i(),t()}),n.querySelector(`.confirm-no`).addEventListener(`click`,()=>{i()})}function G(e,t=!1){let n=document.getElementById(`status-msg`);n&&(n.textContent=e,n.className=t?`error`:`success`,t||setTimeout(()=>{n.textContent===e&&(n.textContent=``)},2e3))}function K(e){let t=document.createElement(`div`);return t.textContent=e,t.innerHTML}var q=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>`,J=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>`,Y=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>`,X=`<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>`;function Z(e,t,n,r){return`https://${e}/${t}/${n}/issues/${r}`}function Q(e){return _?_.hashTarget(e):`_blank`}function $(e){return e.replace(/&/g,`&amp;`).replace(/"/g,`&quot;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`)}N();